...
def sentence1() -> Expr:
    ...
    not_b_or_c = not_b | C
    s2 = not_a % not_b_or_c
    s3 = disjoin([not_a, not_b, C]) 
    return conjoin([a_or_b, s2, s3])
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    combine = itertools.combinations(literals, 2)
    lst = []
    for c in combine:
        lst.append(disjoin([~c[0], ~c[1]]))
    return conjoin(lst)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for pos in all_coords:
        s1 = PropSymbolExpr(wall_str, pos[0], pos[1])
        s2 = ~PropSymbolExpr(pacman_str, pos[0], pos[1], time=t)
        pacphysics_sentences.append(s1 >> s2)
    pac_possible_pos = [PropSymbolExpr(pacman_str, p[0], p[1], time=t) for p in non_outer_wall_coords]
    pacphysics_sentences.append(exactlyOne(pac_possible_pos))
    pac_possible_action = [PropSymbolExpr(action, time=t) for action in DIRECTIONS]
    pacphysics_sentences.append(exactlyOne(pac_possible_action))
...
def positionLogicPlan(problem) -> List:
    ...
    for i in range(50):
        print(i)
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=i) for x, y in non_wall_coords]))
        goal_assertion = PropSymbolExpr(pacman_str, xg, yg, time=i)
...
def foodLogicPlan(problem) -> List:
    ...
    for i in range(50):
        print(i)
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=i) for x, y in non_wall_coords]))
        goal_assertion = [~PropSymbolExpr(food_str, x, y, time=i) for x, y in food]
        model = findModel(conjoin(KB) & conjoin(goal_assertion))
        if model != False:
            return extractActionSequence(model, actions)
        KB.append(exactlyOne([PropSymbolExpr(action, time=i) for action in actions]))
        for x, y in non_wall_coords:
        ...
        for x, y in food:
            KB.append(foodSuccessorAxiom(x, y, i))
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"

def foodSuccessorAxiom(x, y, time):
    f_t = PropSymbolExpr(food_str, x, y, time=time)
    pac_t = PropSymbolExpr(pacman_str, x, y, time=time)
    f_t1 = PropSymbolExpr(food_str, x, y, time=time + 1)
    return (f_t & ~pac_t) % f_t1
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            ...
            if findModel(conjoin(KB) & pac_pos) is not False:
                possible_locations.append((x, y))

            if a := entails(conjoin(KB), pac_pos):
                KB.append(pac_pos)
            if not_a := entails(conjoin(KB), ~pac_pos):
                KB.append(~pac_pos)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            wall_pos = PropSymbolExpr(wall_str, x, y)
            
            if a := entails(conjoin(KB), wall_pos):
                KB.append(wall_pos)
                known_map[x][y] = 1
            if not_a := entails(conjoin(KB), ~wall_pos):
                KB.append(~wall_pos)
                known_map[x][y] = 0
