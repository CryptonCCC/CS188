...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for var in [PAC, GHOST0, GHOST1]:
        values = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
        variableDomainsDict[var] = values
    for var in [OBS0, OBS1]:
        maxManhattanDis = manhattanDistance((0, 0), (X_RANGE - 1, Y_RANGE - 1))
        values = [i for i in range(maxManhattanDis + MAX_NOISE + 1)]
        variableDomainsDict[var] = values
    "*** END YOUR CODE HERE ***"
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for hiddenVariable in eliminationOrder:
            ...
            if len(joinedFactor.unconditionedVariables()) > 1:
                currentFactorsList.append(eliminate(joinedFactor, hiddenVariable))
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        predictBliefs = DiscreteDistribution()
        for oldPos in self.allPositions:
            if self.beliefs[oldPos] == 0:
                continue
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos in newPosDist:
                predictBliefs[newPos] += newPosDist[newPos] * self.beliefs[oldPos]
        self.beliefs = predictBliefs
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for particleIndex in range(self.numParticles):
            self.particles.append(legalPositions[particleIndex % len(legalPositions)])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in set(self.particles):
            w = self.getObservationProb(observation, pacPos, particle, jailPos)
            beliefDist[particle] *= w
